...
class BaseModel(object):
    def __init__(self, args, loaders, samplers):
        ...
        self.loader = loader
        self.model = TemporalGNN_autov2(args, loader, loader.n_time)
        # self.model = GNN_auto(args, loader)
        # self.model = TemporalNonlinear(args, loader, loader.n_time)
        if args.gpu > -1:
    ...
    def saveModelToFiles(self, args, best_metric, deleteLastFile=True):
        if args.val_num == -1:
            savePath = f'{self.args.data_path}saveModel/self/topk_{self.args.topk}_layer_{self.args.layer}_{best_metric}.pt'
        else:
            savePath = f'{self.args.data_path}saveModel/self/topk_{self.args.topk}_layer_{self.args.layer}_valNum_{self.args.val_num}_{best_metric}.pt'
    ...
    def prepareData(self, batch_data):
        subs, rels, objs, times, batch_idxs, abs_idxs, query_sub_idxs, query_obj_idxs, edge_batch_idxs, batch_sampled_edges = batch_data
        subgraph_data = [batch_idxs, abs_idxs, query_sub_idxs, query_obj_idxs, edge_batch_idxs.to(self.device), batch_sampled_edges.to(self.device)]
        subs = subs.to(self.device).flatten()
        rels = rels.to(self.device).flatten()
        objs = objs.to(self.device)
        times = times.to(self.device).flatten()
        return subs, rels, objs, times, subgraph_data
    ...
    def train_batch(self,):        
        ...
        for batch_data in tqdm(self.trainLoader, ncols=50, leave=False):                      
            # prepare data    
            subs, rels, objs, times, subgraph_data = self.prepareData(batch_data)
            
            # forward
            self.model.zero_grad()
            scores, time_loss = self.model(subs, rels, times, subgraph_data)
            # scores = self.model(subs, rels, times, subgraph_data)
            
            # loss calculation
            pos_scores = scores[[torch.arange(len(scores)).to(self.device), objs.flatten()]]
            max_n = torch.max(scores, 1, keepdim=True)[0]
            loss = torch.sum(- pos_scores + max_n + torch.log(torch.sum(torch.exp(scores - max_n),1))) + time_loss*len(scores)
        ...
        valid_mrr, out_str = self.evaluate(eval_test=False)    
    ...
    def evaluate(self, eval_val=True, eval_test=True, verbose=False, rank_CR=False, mean_rank=False):
        ...
        if eval_val:
            ...
            for batch_data in tqdm(self.valLoader, ncols=50, leave=False):      
                ...
                # forward
                scores,_ = self.model(subs, rels, times, subgraph_data, mode='valid')
                # scores = self.model(subs, rels, times, subgraph_data, mode='valid')
                ...
                for i in range(len(subs)):
                    filt = self.loader.filters[(subs[i], rels[i], times[i])]
        ...
        if eval_test:
            ...
            for batch_data in tqdm(self.testLoader, ncols=50, leave=False):        
                ...
                # forward
                scores, _ = self.model(subs, rels, times, subgraph_data, mode='test')
                # scores = self.model(subs, rels, times, subgraph_data, mode='test')
                scores = scores.data.cpu().numpy()
