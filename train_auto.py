...
from utils import *
from PPR_sampler import pprSampler, TemporalpprSampler

parser = argparse.ArgumentParser(description="Parser for the one-shot-subgraph framework")
parser.add_argument('--data_path', type=str, default='data/icews14/')
...
parser.add_argument('--fact_ratio', type=float, default=0.95)
...
if __name__ == '__main__':
    ...
    test_homo_edges = store_by_time(test_data)
    ...
    #     test_homo_edges, test_data, args.data_path, split='test', temporal=True, args=args)
    test_sampler = TemporalpprSampler(loader.n_ent, loader.n_rel, args.n_samp_ent, args.n_samp_edge,
        test_homo_edges, test_data, args.data_path, split='test', temporal=True, args=args)

    del test_homo_edges
        
    # sampler for training
    # fact_homo_edges = list(set([(h,t) for (h,r,t) in loader.train_data]))
    fact_homo_edges = store_by_time(loader.train_data)
    fact_data = np.concatenate([np.array(loader.train_data), loader.idd_data], 0)
    # train_sampler = pprSampler(loader.n_ent, loader.n_rel, args.n_samp_ent, args.n_samp_edge,
    #     fact_homo_edges, fact_data, args.data_path, split='train', temporal=True, args=args)
    train_sampler = TemporalpprSampler(loader.n_ent, loader.n_rel, args.n_samp_ent, args.n_samp_edge,
        fact_homo_edges, fact_data, args.data_path, split='train', temporal=True, args=args)
    ...
    def run_model(params):       
        ...
        # train_when = time.time()
    # # 转换为本地时间并格式化输出
    # train_when = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(train_when))
    # path = './log/{}-{}/'.format(args.dataset, train_when)
    # if not os.path.exists(path):
    #     os.makedirs(path)
    # writer = SummaryWriter(log_dir='log/{}-{}/'.format(args.dataset, train_when))
    ...
    if dataset == 'WN18RR':
        ...
        # exit()
        params = {'lr': 0.001, 'hidden_dim': 32, 'attn_dim': 2, 'n_layer': 8, 'act': 'relu', 'initializer': 'relation', 'concatHidden': True, 'shortcut': False, 'readout': 'multiply', 'decay_rate': 0.9429713470775948, 'lamb': 0.000946516892415447, 'dropout': 0.19456805575101324} 
    run_model(params)