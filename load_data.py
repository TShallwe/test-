...
class DataLoader(Dataset):
    def __init__(self, args, mode='train'):
        ...
        self.entity2id = json.load(open(task_dir + "entity2id.json"))
        self.relation2id = json.load(open(task_dir + "relation2id.json"))
        self.ts2id = json.load(open(task_dir + "ts2id.json"))
        ...
        self.all_triple = np.concatenate([np.array(self.train_triple), np.array(self.valid_triple), np.array(self.test_triple)], axis=0)
        self.fact_triple = np.concatenate([np.array(self.train_triple), np.array(self.valid_triple), np.array(self.test_triple)], axis=0)
        # s1 = set([tuple(tri) for tri in self.train_triple])
        # s2 = set([tuple(tri) for tri in self.valid_triple])
        # s3 = set([tuple(tri) for tri in self.test_triple])
        # add inverse
        ...
        self.idd_data = np.concatenate([np.expand_dims(np.arange(self.n_ent),1), 2*self.n_rel*np.ones((self.n_ent, 1)), np.expand_dims(np.arange(self.n_ent),1), np.zeros((self.n_ent, 1))], 1)
        ...
        self.n_time = max(self.all_triple[:,3])
    ...
    def __getitem__(self, idx):
        ...
        try:
            subgraph = self.getOneSubgraph(int(sub), int(obj), int(tim))
        except:
            subgraph = self.getOneSubgraph(int(sub), int(obj[0]), int(tim))
        return sub, rel, obj, tim, subgraph
    ...
    def collate_fn(self, data):
        ...
        times = torch.stack([_[3] for _ in data], dim=0)
        subgraph_list = [_[4] for _ in data]
        ...
        batch_idxs, abs_idxs, query_sub_idxs, query_obj_idxs, edge_batch_idxs, batch_sampled_edges = batch_subgraph
        return subs, rels, objs, times, batch_idxs, abs_idxs, query_sub_idxs, query_obj_idxs, edge_batch_idxs, batch_sampled_edges
    ...
    def read_triples(self, filename):
        ...
        with open(os.path.join(self.task_dir, filename)) as f:
            for line in f:
                ...
                h, r, t, tim = self.entity2id[h], self.relation2id[r], self.entity2id[t], self.ts2id[tim]
                triples.append([h,r,t, tim])
                self.filters[(h,r,tim)].add(t)
                self.filters[(t,r+self.n_rel,tim)].add(h)
        return triples
    ...
    def double_triple(self, triples):
        ...
        for triple in triples:
            h, r, t, tim = triple
            new_triples.append([t, r+self.n_rel, h, tim]) 
